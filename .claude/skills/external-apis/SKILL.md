---
name: external-apis
description: Nominatim reverse-geocoding and Overpass road-snapping in lib/generate_segments.py. Use when a segment rebuild is suddenly slow (minutes instead of seconds), when segment names are wrong, generic, or show raw coordinates ("Loop near -33.843, 151.193"), when drawn lines ignore roads or zig-zag between parallel streets, when a build must work offline or an OSM API is down, or when touching _name_segments, _reverse_geocode, _match_segments, _snap_poly, or their caches and rate limits. Covers the per-build geocode memo, the 1 req/s Nominatim floor, the single union-bbox Overpass fetch with an in-memory edge grid, location-tolerant cache reuse (NAME_REUSE_M / MATCH_REUSE_M) and why medoid drift makes it necessary, the two-thread build phase, and offline degradation.
---
...
- You are editing naming, snapping, or the caches they write.

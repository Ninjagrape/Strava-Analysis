...
|---|---|---|---|---|
| 1 | `cache/segments_cache.json` (`SEG_CACHE`) | one SHA1 from `_runs_signature` | signature mismatch, or deleting the file | `{"signature": ..., "segments": [...]}`; 73 segments as of 2026-07-02. Names and snapped polylines are BAKED IN (see Pitfalls). |
...
| 5 | `csv_data/YYYY-MM-DD_strava.csv` | activity id (`Filename` column) via `load_prior_fit_cache` in `strava_compile.py` | `--rebuild` only | the enriched CSV IS the .fit parse cache, but as of 2026-07 it is DEAD CODE: `load_prior_fit_cache` reads the prior CSV without raising `csv.field_size_limit`, the oversized stream cells raise `_csv.Error`, the bare `except Exception: continue` swallows it, and the cache loads empty, so every compile is a full re-parse. `--rebuild` stays the correct mental model for forcing intent, and remains mandatory after parse-logic changes because any future fix of the loader re-arms the cache. Full story: `.claude/skills/compile-and-streams/SKILL.md`. |
...
| Added new runs (fresh Strava export) | Nothing. Compile re-parses all .fit files today (the parse cache is dead code, see the inventory), and the new run rows change the signature automatically. |
...
3. CSV-derived values stale (splits, streams, best efforts)? Compile prints `Reused X cached, parsed Y new`, but as of 2026-07 the parse cache is dead code so this reads `Reused 0` every time (see the inventory and `.claude/skills/compile-and-streams/SKILL.md`). Run `python strava_compile.py --rebuild` to force intent, then check whether your change also needs a segments-cache delete (master table).
...
- The parse cache reuses 0 cached .fit rows on every run as of 2026-07: `load_prior_fit_cache` never raises `csv.field_size_limit`, so reading the prior CSV's oversized stream cells throws `_csv.Error`, the bare `except Exception: continue` swallows it, and the cache always loads empty. This is a field-size-limit bug, not filename-keying or a same-day-name collision; every compile is a full re-parse. Full story: `.claude/skills/compile-and-streams/SKILL.md`.

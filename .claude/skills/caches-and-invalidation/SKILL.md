...
| `duplicates` in `cache/config.json`, or a `DUP_*` threshold in `lib/dedupe.py` | Nothing. The params token interpolates the `DUP_*` constants and `load_config().duplicates` under `dedupe1`, so the signature changes and a rebuild follows. The token is required, not belt-and-braces: dropping a run does change the per-run hashes, but the thresholds deciding *which* recording survives do not, and the live NRC/Mi pair is identical on `date_iso`, `dist_km`, polyline length and endpoints, so a winner flip would otherwise be invisible. |
| Anything in `lib/elevation.py` that changes a filled profile (`DEM_ANCHOR_M`, `DEM_SMOOTH_W`, `DEM_KEY_DP`, `DEM_MIN_COVER`, the endpoint) | Bump the `dem1` token in `_runs_signature` AND delete `cache/segments_cache.json`. The backfill lands in `dist_stream`, which the signature does not hash, so detection otherwise keeps using the old profile. Do NOT delete `cache/elevation_dem_cache.json` unless the *endpoint or dataset* changed: it stores raw DEM answers, which anchor spacing and smoothing do not affect, and refetching costs minutes of paced requests. |

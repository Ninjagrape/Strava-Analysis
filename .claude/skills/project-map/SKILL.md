...
1. **The enriched CSV IS the parse cache.** `strava_compile.py` skips .fit files whose activity ids already appear in the latest `csv_data/*_strava.csv` unless `--rebuild` is passed (the `--rebuild` flag's help text: "Ignore the prior CSV cache and re-parse every .fit file"). Consequence: changing any stream/density constant silently does nothing to old rows until you `--rebuild`. Caveat: as of 2026-07 the cache is DEAD CODE, a `csv.field_size_limit` bug in `load_prior_fit_cache` makes it load empty every time, so every compile is a full re-parse (79 files in ~2.8 s with 28 workers); the `--rebuild` mental model still applies, full story in `.claude/skills/compile-and-streams/SKILL.md`. Grep: `grep -n "rebuild" strava_compile.py`.
...
- Reading the enriched CSV with stdlib `csv` fails with "field larger than field limit" unless you first call `csv.field_size_limit(min(sys.maxsize, 2**31 - 1))`. Only `lib/generate_analytics.py` and `lib/generate_dashboards.py` do this today (as of 2026-07): `grep -rn "field_size_limit" .`
...
- `minetti_cost`/`ga_time` are deliberately duplicated in both `lib/generate_analytics.py` and `lib/generate_dashboards.py`; fix bugs in both.

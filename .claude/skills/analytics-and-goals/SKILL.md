...
1. **`lib/generate_analytics.py`, longitudinal fitness.** Per-session pace-based load (`session_loads`) → gap-filled daily series (`daily_series`) → exponentially smoothed CTL/ATL/TSB (`ctl_atl_tsb`), ACWR (`acwr_series`), Foster monotony/strain (`monotony_strain`); plus critical-speed model (`fit_critical_speed`), CS-anchored pace zones, monthly VDOT trend (`vdot_trend`), cadence trend, calendar heatmap. Load is pace-based (grade-adjusted distance × intensity²) and stays that way permanently, so the whole history sits on one comparable curve. **As of 2026-08-16 exactly one metric reads heart rate**: the HR-derived VO2max trend (`hr_vo2max_trend`, own row in the table below). Everything else is still pace/distance/elevation only, and a run without HR degrades by being absent from that one chart. Earlier wording claiming the module is entirely HR-free no longer holds.
...
As of 2026-07-10 the Analytics panel is split into themed sub-tabs (Injury risk / Paces / Trends, `setSubTab` in `generate()`), and `body_analytics(rows, updated, runs=None)` takes the hub's classified runs list to render `pace_progression_section(runs)`: per-run grade-adjusted pace (`ga_time(moving_s, gain, dist_km)/dist_km`) bucketed by `run_type` (types with ≥3 runs, `misc` excluded), scattered over real-date x with a recency-weighted (30-day half-life) trend (`_rolling_trend`, `_pace_prog_chart`; y inverted so faster is up). As of 2026-07-13 `_rolling_trend(dates, paces)` replaced the old count-based trailing 5-run mean: each earlier run's weight halves every `_PACE_PROG_HALFLIFE_DAYS = 30` days of age, so a lone recent attempt of a sparse type is no longer dragged toward months-old runs (the "Current {type} pace" stat = the last trend value, now always defined). The type selector reuses `_ranged`/`setRange`; colours mirror the hub's `RUN_TYPE_COLOR`. The standalone `main()` passes no runs, so that build simply omits the section. Because classification is era-relative (`_threshold_at`), absolute paces for the same label still drift as fitness changes, but the trend no longer averages across distant eras by run count.
...
| HR VO2max | `hr_vo2max_trend`, `run_hr_vo2max`, `hr_reference` | analytics | per-window steady-state: GA speed via `minetti_cost` on the window's own net grade → `daniels_vo2_at_velocity` → `VO2_REST + (VO2 − VO2_REST) / %HRR`; run = 75th pctile of windows, month = best run | `HRV_*` block + `HR_REST_DEFAULT = 60`, `VO2_REST = 3.5`; anchors from `cache/config.json` `physiology.hr_max`/`hr_rest`, else max observed HR and 60 | recreational ~35–60; ±4 units across plausible anchor choices (measured 2026-08-16) |
...
Most numbers above are **inline literals, not module constants**, `42`/`7` in `ctl_atl_tsb`, `0.15` percentile and the `×10`/`²` in `session_loads`, `1200` in `fit_critical_speed`, `1.06` in `riegel`, the zone multipliers in `pace_zones_from_cs`, `0.20` in `is_interval`. Named tunables: `CS_DISTANCES`, `DAILY_RANGES`/`MONTHLY_RANGES`/`CAL_RANGES`, `MAX_AS_OF_DAYS = 7`, and the HR-VO2max block (analytics, as of 2026-08-16): `HRV_WINDOW_S = 150.0`, `HRV_WARMUP_S = 300.0`, `HRV_LAG_S = 20.0`, `HRV_MAX_GAP_S = 20.0`, `HRV_MIN_SPEED_MPS = 2.0`, `HRV_MAX_SPEED_MPS = 7.0`, `HRV_MAX_GRADE = 0.15`, `HRV_STEADY_RATIO = 1.35`, `HRV_HRR_MIN = 0.50`, `HRV_HRR_MAX = 0.95`, `HRV_MIN_HR_PTS = 3`, `HRV_MIN_WINDOWS = 4`, `HRV_RUN_PCTILE = 0.75`, `HRV_MIN_VO2 = 25.0`, `HRV_MAX_VO2 = 90.0`, `HRV_GRADE_LIMIT = 0.45`, `HR_REST_DEFAULT = 60`, `HR_MAX_FLOOR = 120`, `HR_MAX_CEILING = 230`, `VO2_REST = 3.5`; `TRAINING_DEFS`, `DISTANCE_BANDS`, `WEEKLY_RANGES`, `_RACE_BAR_HALF_WINDOW_S_PER_KM = 60.0` (dashboards); `THRESHOLD_WINDOW_DAYS = 60` and the `_classify_run` share constants (hub). No cache guards any of this, analytics/dashboards recompute from the CSV on every hub run, so edits take effect immediately (only segments are cached).
...
- HR VO2max wiring: `rg -n "HRV_|hr_vo2max_trend|hr_reference|_hr_vo2max_section" lib/generate_analytics.py`, and `rg -n "physiology" lib/config.py config.example.json`
- HR VO2max baseline (as of 2026-08-16, 95 primary runs, 5 of them with HR, no `physiology` config): anchors 186/60 bpm, all 5 runs qualify over 45 steady windows, monthly series `[("Jul", 50.9), ("Aug", 50.3)]` against a VDOT series ending `[("Jul", 41.9), ("Aug", 45.4)]`. The HR figure sitting above VDOT is expected: those best efforts were training runs, not races.
...
- **HR feeds exactly one metric**: `hr_vo2max_trend` and nothing else (as of 2026-08-16). Load, CTL/ATL/TSB, ACWR, monotony and the pace zones stay pace-based on purpose, so a strap arriving partway through the history does not put a step in any of those curves. Do not "fix" a load number by reaching for HR without redesigning the load model.
- **HR VO2max needs the hub's `runs`, not CSV rows**: it reads per-point `dist_stream` HR and, crucially, elevation that `backfill_elevation` attaches at hub time. The standalone `python lib/generate_analytics.py` build passes no runs and omits the section entirely (same convention as `pace_progression_section`). A DEM run in the hub that predates `backfill_elevation` in the call order would silently fall back to the run's average grade.
- **Its absolute level is an assumption, its trend is not**: with no `physiology` block in `cache/config.json` it anchors on the highest bpm ever recorded (understates a true max) and an assumed 60 resting. Measured 2026-08-16 on 5 HR runs, plausible anchor choices moved the monthly figures over a ~4-unit range and could reorder two months that sat 0.6 apart. Treat a sub-unit month-on-month move as noise.
